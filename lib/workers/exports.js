...
// Run a mongoose query as a promise. The old Q.nsend(model, method, ...args)
// form passed a node callback, which double-executes the query on mongoose 6
// ("Query was already executed"). Call the method and await its .exec() instead.
...
function processBulkExport(job) {
  ...
  return runQuery(Export.model || mongoose.model('Export'), 'findByIdAndUpdate', exportId, { status: 'processing' })
    ...
    .then(function(foundUser) {
      ...
      return runQuery(Trinket.model || mongoose.model('Snippet'), 'count', { _owner: userId });
    })
    .then(function(count) {
      // Update total count
...
function createExportArchive(userId, exportId, tempFile) {
  ...
  // Stream trinkets via a cursor (.stream() was removed in mongoose 5+;
  // .cursor() returns a readable stream with the same data/end/error events)
  ...
  return resolveStudent(ctx.course, group._id)
    .then(function(student) {
      return Q(Trinket.findById(current.trinketId)).then(function(subTrinket) {
        ...
        var slugSource = student.username ||
          sanitizeFolderName((student.email || '').split('@')[0]) ||
          String(group._id);
