var config      = require('config'),
    path        = require('path'),
    _           = require('underscore'),
    lodash      = require('lodash'),
    moment      = require('moment'),
    numeral     = require('numeral'),
    nunjucks    = require('nunjucks'),
    deployDir   = require('../../config/deploy-dir'),
    // Deploy views (deploys/<name>/views) shadow stock templates by name.
    _templates  = deployDir.sub('views')
                    ? [deployDir.sub('views'), config.app.templates]
                    : config.app.templates,
    _loader     = new nunjucks.FileSystemLoader(_templates, {watch: config.isDev || config.isTest}),
    env         = new nunjucks.Environment(_loader, {autoescape: true}),
    StringUtils = require('./stringUtils'),
    AssetVersion = require('./assetVersion'),
    cachify     = require('./cachify'),
    translate   = require('./translate'),
    roles       = require('./roles'),
    component   = require('./component'),
    constants   = require('../../config/constants');

// Lock loaders against external replacement — something in the startup
// sequence reassigns env.loaders directly, wiping the FileSystemLoader.
var _loaders = env.loaders.slice();
Object.defineProperty(env, 'loaders', {
  get: function() { return _loaders; },
  set: function() {},   // silently ignore replacements
  configurable: true
});

// components/ gets its own CONTENT hash rather than the deploy commit (#238):
// it changes a few times a year, so re-issuing its URLs every deploy re-sent
// ~6.6 MB per student for nothing. Everything else stays deploy-scoped.
function tokenFor(src, key) {
  var kind = key || String(src || '').split('/')[1];
  return kind === 'components' ? AssetVersion.componentsToken() : AssetVersion.token();
}

env.addFilter('cachePrefix', function(src, key) {
  return StringUtils.addPrefix(src, config.app.prefixes, key, tokenFor(src, key));
});

// The same stamping as the filter above, as a plain function, for callers that
// emit script tags from a list rather than a template expression (cachify_js).
// Every embed template builds its script list this way, and cachify used to
// emit the list bare -- ~21 requests / ~139 KiB per embed view on a 5-minute
// TTL while the filtered references beside them were immutable (#234).
function stampSrc(src) {
  return StringUtils.addPrefix(src, config.app.prefixes, undefined, tokenFor(src));
}

// The client builds asset URLs too (trinket-config.js assetPath). It must use
// the same token, or client-built URLs go on busting the cache per call.
env.addGlobal('assetToken', function() {
  return AssetVersion.token();
});
// The glowscript runner builds its own component URLs, so it needs the
// components token — not the deploy token — or those 6.6 MB roll every deploy.
env.addGlobal('componentsToken', function() {
  return AssetVersion.componentsToken();
});
env.addFilter('json', function(str, opt) {
  if (opt === 'pretty') {
    return JSON.stringify(str, null, 2);
  } else {
    return JSON.stringify(str);
  }
});
env.addFilter('translate', function(str, locale) {
  return translate(str, locale);
});

// "New Trinket" menu items, in display order. Single source of truth so the nav
// dropdown and the single-type shortcut button (base.html) can't drift. Note
// python3 shares the python icon. Intentionally distinct from the client-side
// trinketTypes array in base.html (which also carries pyodide as its own entry).
// 'pyodide' is intentionally omitted: new Python trinkets are created as
// 'python3' (the canonical Python type); pyodide is the runtime + legacy lang.
// Feature flags keep 'python' (Skulpt) and 'python3' from showing together.
var NEW_TRINKET_MENU = [
  { lang: 'python',     label: 'Python',      icon: 'lang-sprite-python' },
  { lang: 'python3',    label: 'Python',      icon: 'lang-sprite-python' },
  { lang: 'pygame',     label: 'Pygame',      icon: 'lang-sprite-pygame' },
  { lang: 'R',          label: 'R',           icon: 'lang-sprite-R' },
  { lang: 'blocks',     label: 'Blocks',      icon: 'lang-sprite-blocks' },
  { lang: 'html',       label: 'HTML',        icon: 'lang-sprite-html' },
  { lang: 'glowscript', label: 'Web VPython', icon: 'lang-sprite-glowscript' },
  { lang: 'java',       label: 'Java',        icon: 'lang-sprite-java' },
  { lang: 'music',      label: 'Music',       icon: 'lang-sprite-music' }
];
// A type shows unless features.trinkets explicitly disables it (matches the old
// inline template conditionals). Pass config.features from the template.
env.addGlobal('enabledNewTrinketTypes', function(features) {
  return NEW_TRINKET_MENU.filter(function(t) {
    return !features || !features.trinkets || features.trinkets[t.lang];
  });
});
env.addFilter('userAvatar', function(str) {
  if (!str) {
    return '/img/avatar-default.svg';
  }
  // Already a full URL
  if (/^http/.test(str)) {
    return str;
  }
  // Already a local path
  if (/^\//.test(str)) {
    return str;
  }
  // Relative path - prepend cloud host if configured
  var cloudHost = config.aws.buckets.useravatars.host || '';
  if (cloudHost.length > 0 && !cloudHost.includes('example.com')) {
    return cloudHost + '/' + str;
  }
  // Default to local img path
  return '/img/' + str;
});
env.addFilter('encrypt', function(obj) {
  return roles.encrypt(obj);
});
function escapeJSON(data) {
  if (typeof data === 'undefined' || data === null) {
    return null;
  }

  if (data instanceof Array) {
    for (var i = 0; i < data.length; i++) {
      data[i] = escapeJSON(data[i]);
    }
  }
  else if (typeof data === 'object') {
    for (var i in data) {
      if (data.hasOwnProperty(i)) {
        data[i] = escapeJSON(data[i]);
      }
    }
  }
  else if (typeof data === 'string') {
    // lodash and underscore can produce different results
    // lodash is used on the client-side so we'll use it here too
    data = lodash.escape(data);
  }

  return data;
}
env.addFilter('escapeJSON', function(obj) {
  var e = escapeJSON(obj);
  return e;
});

module.exports = {
  render: function(template, context) {
    if (config.isDev || config.isTest) {
      env.cache = {};
    }
    // Render through this module's `env` (which owns the FileSystemLoader for
    // config.app.templates) — the global nunjucks has no app loader at startup,
    // so it can't find templates like emails/passwordReset (#219). Keep the
    // Promise return: callers in courseInvitation.js and courses.js chain
    // .then() on the result, and the users.js callers await it.
    return new Promise(function(resolve, reject) {
      env.render(template, context, function(err, result) {
        if (err) return reject(err);
        resolve(result);
      });
    });
  },
  compile: function(src, info) {
    // Vision passes src (template source string) and info.filename (absolute path)
    // Extract template name relative to templates directory for nunjucks.render()
    // We need to convert absolute path to relative path from templates directory
    var templatesDir = path.resolve(config.app.templates);
    var templateName = info.filename.replace(templatesDir, '');
    // Remove leading slash if present
    if (templateName.charAt(0) === '/' || templateName.charAt(0) === '\\') {
      templateName = templateName.substring(1);
    }

    var subdomain = function(instructor, course) {
      if (config.app.usersubdomains) {
        return '/' + course.slug;
      }
      else {
        return ['', 'u', instructor.slug, 'classes', course.slug].join('/');
      }
    };

    return function(context) {
      // kill the nunjucks cache when in dev mode
      if (config.isDev || config.isTest) {
        env.cache = {};
      }

      // Use the request hostname (_hostname injected by onPreResponse) so that
      // templates work correctly regardless of which domain served the request
      // (custom domain, tagged candidate URL, localhost, etc.).
      var hostname = context._hostname || config.app.url.hostname;
      var renderConfig = config;
      if (hostname !== config.app.url.hostname) {
        var urlOverride = Object.create(config.app.url);
        urlOverride.hostname = hostname;
        var appOverride = Object.create(config.app);
        appOverride.url = urlOverride;
        renderConfig = Object.create(config);
        renderConfig.app = appOverride;
      }

      var host = function(instructor) {
        var url = renderConfig.app.url.protocol + '://';
        if (renderConfig.app.usersubdomains && instructor) {
          url += instructor.slug + '.';
        }
        url += hostname;
        return url;
      };

      _.extend(context, {
        config     : renderConfig,
        moment     : moment,
        numeral    : numeral,
        subdomain  : subdomain,
        host       : host,
        cachify_js : function(key, srcList) { return cachify.js(key, srcList, stampSrc); },
        translate  : translate,
        component  : component,
        constants  : constants
      });

      return env.render(templateName, context);
    };
  },
  stampSrc : stampSrc,
  env : env
}
