...
function markReported(submission, sourcedId) {
  if (!submission || !sourcedId || !submission.id) return Promise.resolve(null);
  var at = new Date();
  // Write ONLY the two marker fields, never the whole row.
  //
  // submission was loaded BEFORE the LMS round trip, and on Firestore save() is
  // doc.set(data) -- a whole-document overwrite. controllers/course.js
  // updateMySubmission edits this same row in place (code, assets, settings,
  // submittedOn, comments), so a resubmission landing during the post was
  // reverted to the pre-post snapshot: the student's work, not bookkeeping, and
  // invisible because save() stamps lastUpdated like any ordinary edit.
  //
  // findByIdAndUpdate is doc.set(patch, {merge:true}) on firestore and a $set on
  // mongo, so the untouched fields stay untouched on both.
  //
  // ⚠️ Mongo alone could not show this: mongoose save() writes only modified
  // paths, so the regression test passes on the mongo leg and fails on
  // firestore. See test/lib/models/trinket-mark-reported.test.js.
  //
  // Still best-effort and still AFTER the post: if this write fails we simply
  // report again next launch, which is harmless, whereas marking before the
  // post would lose the submission for good if the post then failed.
  //
  // Deliberately does NOT stamp the caller's in-memory object: nothing reads it
  // back after the post, and mutating a shared reference is a side effect a
  // best-effort bookkeeping call has no business having.
  function done() { return null; }
  return Promise.resolve()
    .then(function () {
      return Trinket.findByIdAndUpdate(submission.id,
        { $set: { ltiReportedSourcedId: sourcedId, ltiReportedAt: at } });
    })
    .then(done, done);
}
...
function notifyOnCoordinates(platformId, resourceLinkId, userId, sourcedId) {
  return Promise.resolve().then(function () {
    ...
    // One document off the index, rather than every revision this student has
    // for the material filtered in memory: on Firestore each returned document
    // is a billed read, and a student who iterates pays one per revision on
    // EVERY graded launch. findNewestSubmission also orders by submittedOn
    // rather than created, which is the order that actually matters here --
    // updateMySubmission can stamp an older document after a newer attempt.
    return Promise.resolve(Trinket.findNewestSubmission(userId, link.targetId))
...
module.exports = {
  notify: notify,
  // Test seam: the concurrent-write guarantee is pinned against the real
  // backends in test/lib/models/trinket-mark-reported.test.js.
