...
function FileUtil() {
  ...
  // Signed-URL direct-to-storage upload path for large course archive imports
  // (bypasses the 32 MiB Cloud Run ingress cap on direct POST). Available only
  // when the deploy has opted in AND the storage backend supports presigned
  // URLs (S3-compatible or GCS).
  ...
  this.importObjectSize = function(key) {
    return backend.objectSize(config.aws.buckets.materials.name, key);
  };
