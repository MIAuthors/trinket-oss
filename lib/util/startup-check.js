...
// docs/history/GCR-PICUP-TRIAL-MERGE-NOTES.md): choosing GCP means firestore AND
...
async function run() {
  ...
  checks.push(rateLimitScope(config));
...
// What "rate limited" actually means on this deploy.
//
// The limiter's backing store decides its scope, and the two we ship differ in
// opposite directions:
//   * Redis  — one counter for the whole deploy. maxPerAccount really is the
//              ceiling. (This is the store #248's TTL bug applied to.)
//   * memory — module-level, so one counter PER PROCESS. On Cloud Run with
//              maxScale: 10 the effective ceiling is up to 10x the configured
//              number, and a scale-to-zero resets every counter.
// Neither is wrong, but `maxPerAccount: 10` reads like a promise, and on the
// in-memory path it is not one. Say so where the operator already looks.
function rateLimitScope(cfg) {
  var enabled = !!(cfg && cfg.db && cfg.db.redis && cfg.db.redis.enabled);
  return enabled
    ? '  Limits:  redis      (shared across instances — configured maxima are real)'
    : '  Limits:  in-memory  (per-instance counters; the real ceiling is the\n' +
      '           configured maximum times the instance count, and a cold\n' +
      '           start resets it. Set db.redis.enabled for a shared limiter.)';
}

module.exports = { run, checkShape, checkDependencies, rateLimitScope };
