...
function toSubmissionView(trinket) {
  ...
  var view = {
    ...
    , submissionState : trinket.submissionState
      // The dashboard rows carry `state` (from the per-material aggregation) and the
      // directive switches on it: ng-switch="state" is what decides whether the
      // feedback form renders at all. Without it the panel shows the submission and
      // silently offers no way to respond.
    , state           : trinket.submissionState
  };
