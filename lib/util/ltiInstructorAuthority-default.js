...
// Sync, display-only companion to resolveInstructor: may this user be SHOWN the
// "Connect LMS" link? The nav renders synchronously, so it cannot await the
// async gate. Mirrors resolveInstructor's email arm exactly and, like the route
// gate (helpers.canInitiateLtiRegistration), passes no lmsTeacher — so a
// trust-the-platform deploy with no list fails closed. The route remains
// authoritative; this only decides whether the link is drawn.
