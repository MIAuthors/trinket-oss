...
InMemoryQueue.prototype._processJob = function(job) {
  ...
  if (this.handlers.length === 0) {
    ...
    // Generic on purpose: this module knows nothing about which feature's
    // worker was missing. Callers that enqueue user-visible work should
    // check hasHandlers() up front (lib/util/exportGuard.js does).
    console.warn('Queue [' + self.name + '] DROPPED job ' + job.id +
      ' — no handler registered in this process; work queued here is discarded.');
    return;
  }
