...
// Detect a Hapi response object (from h.response()/h.view()). request.success
// must never serialize one: a Response holds a back-reference to the whole
// request/server graph, so ObjectUtils.serialize would walk that huge, circular
// structure and exhaust the heap. This happens when a handler returns
// request.fail(...) from inside a promise chain and a later .then feeds that
// Response into request.success (picup #58 / M&I #7 crash-loop). Multi-signal
// match to avoid false positives on ordinary payloads.
function isHapiResponse(v) {
  return !!(v && typeof v === 'object'
    && v.constructor && v.constructor.name === 'Response'
    && typeof v.statusCode !== 'undefined'
    && 'variety' in v && 'source' in v && 'request' in v);
}
...
function parseRoutes(routeConfigs) {
  ...
  routeConfigs.forEach(function(route, index) {
    ...
    // Stash the route's fail view where a response-time hook can reach it.
    // `fail.html` was only ever consulted by request.fail() — i.e. a controller
    // choosing to fail — so a PRE-handler that throws (isAdmin -> Boom.forbidden)
    // bypassed it entirely and fell through to the generic error page. A route
    // that declares `fail: { html: 'login.html' }` clearly means that page to be
    // the failure surface for the route, gate included (issue #74).
    if (fail && fail.html) {
      route.config = route.config || {};
      route.config.app = route.config.app || {};
      route.config.app.failHtml = fail.html;
    }
    ...
    route.handler = async function(request, h) {
      ...
      try {
        ...
        request.success = function(json) {
          ...
          // Defense-in-depth (#58): never serialize a Hapi Response. If a
          // handler accidentally fed one back here (e.g. request.fail(...)
          // returned from an earlier .then, then a trailing .then calls
          // request.success), serializing it would walk the whole
          // response->request->server graph and blow the heap. The response was
          // already resolved by request.fail; pass it straight through.
          if (isHapiResponse(json)) { responseResolver(json); return json; }
          if (json && typeof json === 'object') {
            for (var _rk in json) {
              if (isHapiResponse(json[_rk])) { responseResolver(json[_rk]); return json[_rk]; }
            }
          }
        ...
        request.fail = function(json, err) {
          ...
          // If json is an Error, Hapi refuses h.response(Error) — convert to Boom
          if (json instanceof Error) {
            response = Boom.badImplementation(json.message);
            responseResolver(response);
            return response;
          }
...
function addStaticRoutes(routes) {
  // Deploy public assets (deploys/<name>/public) shadow ./public by name —
  // inert tries each path in order and serves the first match.
  var deployPublic = require('../../config/deploy-dir').sub('public');
  var publicPaths  = deployPublic ? [deployPublic, './public'] : './public';
    ...
    handler: {
      directory: {
        path: function(request) {
          var sub = '/' + request.params.assetType;
          return deployPublic
            ? [deployPublic + sub, './public' + sub]
            : './public' + sub;
        },
...
function addUserContext(json, request) {
  ...
  var hasMailgun = config.app.mail && config.app.mail.from &&
                 (config.app.mail.key || config.app.mail.pass);
  var hasFrom = config.app.mail && config.app.mail.from;
  json.emailEnabled = hasFrom && (hasAWS || hasMailgun);

  // Can this user reach the import page? Mirrors the gate on GET /account/import
  // (lib/controllers/users.js) so the nav link and the route agree — a link that
  // silently redirects to /account/profile is worse than no link. Required by the
  // layout's Import entry (issue #6: import was reachable only by digging through
  // Account settings, which a user reported as undiscoverable).
  // Required lazily: helpers pulls in the model layer, and this module is loaded
  // during route construction.
  json.canImport = require('./helpers').userCanCreateCourse(request.user);

  // Same contract as canImport, for the "Connect LMS" entry: the nav link and
  // the gate on /lti/connect must agree. See helpers.userCanConnectLms.
  json.canConnectLms = require('./helpers').userCanConnectLms(request.user);

  // Same contract again, for the account bulk-export button: a control that
  // cannot possibly work should say so before it is pressed. On Cloud Run the
  // export worker never registers, so the queue discards the job and the user
  // watched a spinner forever (#199). Course/assignment student-work exports do
  // NOT depend on this — they now build in-request and always work.
  json.exportsAvailable = require('./exportGuard').exportsAvailable(
    require('./queues').exports());
...
module.exports = {
  ...
  // Exported so the onPreResponse hook that renders a route's `fail.html` for a
  // rejected PRE-handler can build the same template context every other view
  // gets (user, flash-adjacent config, emailEnabled — login.html needs them).
