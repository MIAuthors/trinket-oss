...
// `stored > new Date()` happens to WORK for a numeric `stored` (the relational
// operator coerces the Date via valueOf), but silently yields false for a
// string or Timestamp-husk value — which would report a finished export as not
// downloadable. Route every comparison through toDate so the shape never
// matters.
