...
function recordView(userId) {
  ...
  update = {
    "$set" : {
      "users.$.lastView.viewedOn" : new Date()
    }
  };
...
// Applies the settings the payload PROVIDES; a key the payload omits keeps the
// value the course already has (or the schema default if it never had one).
//
// It used to walk the schema and reset every omitted key to its DEFAULT, which
// made every partial update a trap: the Edit Course form sends only the fields
// it shows, so a name-only save silently reset courseType to 'public' and — the
// case that surfaced it — downgraded markdownEngine back to 'legacy'. Pinned by
// 'partial updates must not reset course settings' in
// test/lib/api/course-markdown-engine.test.js (verified RED against the old
// walk).
function setGlobalSettings(settings) {
  ...
  for (var setting in schema.globalSettings) {
    if (schema.globalSettings.hasOwnProperty(setting)) {
      ...
      else if (typeof this.globalSettings[setting] === 'undefined') {
        this.globalSettings[setting] = schema.globalSettings[setting].default;
      }
      // else: leave the existing value alone
    }
  }
}
...
function userHiddenFromDashboard(user) {
  var hideFrom = user.hideFrom || [];
  return hideFrom.indexOf('dashboard') >= 0 || hideFrom.indexOf('all') >= 0;
}
