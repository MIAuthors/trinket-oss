...
function createModel(modelName, config) {
  ...
  // Firestore caps its `in` operator (30 values), and the firestore backend maps
  // $in straight onto it — so an unchunked findByIds silently fails at exactly
  // the scale batching exists to serve. Mongo has no such limit, which is why a
  // test for this passes on the mongo leg and only means anything on firestore.
  // Chunk here rather than at each call site: this is the API CLAUDE.md tells
  // callers to prefer over per-item lookups, so the limit must not be theirs to
  // remember.
  var IN_CHUNK = 30;
  ...
  if (!classMethods.findByIds) {
    classMethods.findByIds = function(ids, cb) {
      ...
      var runChunk = function(chunk) {
        return Promise.resolve(defaultFields
          ? self.model.find({_id:{$in:chunk}}, defaultFields)
          : self.model.find({_id:{$in:chunk}}));
      };

      // Chunks run concurrently; order across chunks is not guaranteed, which
      // matches what an $in query already promised (namedTrinketList, the only
      // other caller, re-sorts its own results).
      var promise = Promise.all(chunks.map(runChunk)).then(function(results) {
        return results.reduce(function(all, batch) { return all.concat(batch || []); }, []);
      });

      if (typeof cb === 'function') {
        promise.then(function(docs) { cb(null, docs); }, function(err) { cb(err); });
        return;
      }
      return promise;
    }
  ...
  if (!classMethods.findById) {
    classMethods.findById = function(id, cb) {
      ...
      // Mongoose resolves findById(undefined/null) to null; Firestore throws
      // on undefined query values, so short-circuit before hitting the backend
      // (e.g. imported legacy-ref trinkets have no _owner)
      if (id == null) {
        promise = Promise.resolve(null);
        promise.exec = function() { return promise; };
        if (cb) {
          promise.then(function() { cb(null, null); });
          return;
        }
        return promise;
      }
      ...
      if (alternateIds && alternateIds.length) {
        ...
        } else if ((appConfig.db && appConfig.db.backend) === 'firestore') {
          // Firestore document IDs are arbitrary strings. Mongoose would
          // CastError on a non-ObjectId _id, so only match a raw-string _id on
          // the firestore backend; on Mongo the alternateIds below cover it.
  ...
  if (!classMethods.findByIdAndUpdate) {
    classMethods.findByIdAndUpdate = function(id, update, options, cb) {
      ...
      // options is genuinely optional: findByIdAndUpdate(id, update) is the
      // natural two-argument call and threw on `options.select` below. Nothing
      // hit it because the callers that update-without-options reached for the
      // raw model instead — which is exactly the habit this wrapper exists to
      // replace.
      options = options || {};
  ...
  if (!classMethods.find) {
    classMethods.find = function(query, fields, cb) {
      return this.model.find(query, fields, cb);
    };
  }

  if (!classMethods.findOne) {
    classMethods.findOne = function(query, fields) {
