var config       = require('config'),
    ...
    clientIp     = require('../util/clientIp'),
...
module.exports = {
  ...
  login : async function(request, reply) {
    // Brute-force protection: limit only FAILED login attempts, keyed by client
    // IP and by target account over a rolling window. Successful logins are NOT
    // counted, so a whole class logging in legitimately — often behind one
    // campus NAT / proxy IP — never trips the limit; only repeated failures do
    // (see issue #3, where the old "count every attempt" design 429'd students
    // on their first try during an exam).
    var rl = (config.app && config.app.auth && config.app.auth.loginRateLimit) || {};
    var WINDOW      = rl.windowSeconds || 900; // 15 min
    var MAX_IP      = rl.maxPerIp      || 100; // failed attempts per IP (generous: shared NAT)
    var MAX_ACCOUNT = rl.maxPerAccount || 10;  // failed attempts per account (the real guard)

    // Behind a proxy, remoteAddress is the proxy IP shared across all users, so
    // keying on it collapses everyone into one bucket. Use the forwarded client
    // IP — and take the RIGHTMOST X-Forwarded-For entry (see lib/util/clientIp):
    // the leftmost entries are client-supplied and spoofable, which would let an
    // attacker forge a fresh rate-limit bucket per request and bypass the limit.
    var ip = clientIp(request);

    var ipKey   = 'rate:login:fail:ip:'   + ip;
    var acctKey = 'rate:login:fail:acct:' + request.payload.email;

    // Read-only gate: reject if this IP or account has already exhausted its
    // failed-attempt budget. Does not increment — only the failure branches do.
    var ipFails   = parseInt(await Store.get(ipKey)   || '0', 10);
    var acctFails = parseInt(await Store.get(acctKey) || '0', 10);
    if (ipFails >= MAX_IP || acctFails >= MAX_ACCOUNT) {
      return reply(errors.tooManyRequests('Too many login attempts. Please try again later.'));
    }

    // Record one failed attempt against both buckets. Store.bump does the
    // increment and sets the window on first use; see its comment for why this
    // is not GET/+1/SET (#248: Redis SET discards the TTL, so the old shape made
    // the lockout permanent, and the read-modify-write did not constrain
    // concurrent attempts). Call before returning any failure below.
    async function recordFailedLogin() {
      await Store.bump(ipKey,   WINDOW);
      await Store.bump(acctKey, WINDOW);
    }
    ...
    try {
      ...
      // Seed the site 'admin' role from the ADMIN_EMAILS allowlist, exactly as the
      // Firebase and Google-OAuth paths do (lib/controllers/auth.js). Without this
      // a local-auth deploy — where this is the ONLY login path — can never
      // bootstrap an admin: every gate downstream reads hasRole('admin'), and
      // nothing else ever stamps it (issue #74).
      //
      // Idempotent: ensureSeedAdminRole writes only when the role is actually
      // added, so the ordinary non-admin login performs no extra write. Failure
      // must not block a valid login, so a rejection is logged and swallowed.
      try {
        await siteAdmin.ensureSeedAdminRole(user);
      } catch (e) {
        console.error('LOGIN: admin role seeding failed (continuing):', e && e.message);
      }
      ...
      if (result.success) {
        User.findByLogin(request.payload.email, function(err, user) {
          ...
          require('crypto').randomBytes(48, async function(ex, buf) {
            ...
            var message = await nunjucks.render('emails/passwordReset', {
              fullname           : user.fullname,
              username           : user.username,
  ...
  savePassword : async function(request, reply) {
    // Forwarded client IP, not the shared proxy socket address (see clientIp).
    var ip = clientIp(request);
    var rateLimitKey = 'rate:save-pass:' + ip;
    // Same GET/+1/SET shape as the login limiter, with the same two faults on a
    // Redis-backed deploy (#248): SET cleared the TTL so the window never
    // elapsed, and the read-modify-write raced. Losing this one strands a user
    // out of password reset permanently, and independently of login.
    var attempts = await Store.bump(rateLimitKey, 900);   // 15-minute window
    if (attempts > 10) return reply(require('@hapi/boom').tooManyRequests('Too many password reset attempts'));
  ...
  account : function(request, reply) {
    ...
    // Importing courses/trinkets is an instructor capability; keep students out
    // of the import settings page even via a direct URL.
    if (request.params.accountPage === 'import' && !helpers.userCanCreateCourse(request.user)) {
      return reply().redirect('/account/profile');
    }

    var canImport = helpers.userCanCreateCourse(request.user);
  ...
  assetUploadFromURL : function(request, reply) {
    ...
    // Block non-HTTP schemes and private/internal hosts to prevent SSRF.
    if (!requestUrl.protocol || !/^https?:$/.test(requestUrl.protocol)) {
      return request.fail();
    }
    ...
    function isPrivateIP(ip) {
      // Reject loopback, link-local (AWS metadata), and RFC-1918 private ranges.
    ...
    dns.lookup(hostname, function(err, address) {
      if (err || isPrivateIP(address)) return request.fail();
  ...
  requestExport : function(request, reply) {
    ...
    Export.findPendingOrProcessing(userId)
      ...
      .then(function(saved) {
        ...
        // Queue the job
        var refused = exportGuard.failIfUnavailable(exportsQueue, exportRecord, request);
      ...
      .then(function(exports) {
        ...
        var data = exports.slice(0, limit).map(function(exp) {
          return {
            ...
            created: dates.toIso(exp.created),
            expiresAt: dates.toIso(exp.expiresAt),
            downloadAvailable: exp.status === 'completed' && dates.isFuture(exp.expiresAt)
          };
        });
      ...
      Export.findById(exportId, function(err, exportRecord) {
        try {
          ...
          var downloadAvailable = exportRecord.status === 'completed' &&
                                  dates.isFuture(exportRecord.expiresAt);
          ...
          return request.success({
            ...
            data: {
              ...
              created: dates.toIso(exportRecord.created),
              expiresAt: dates.toIso(exportRecord.expiresAt),
  ...
  downloadExport : function(request, reply) {
    ...
    Export.findById(exportId, function(err, exportRecord) {
      ...
      // Generate a fresh signed URL from the SAME backend the archive was
      // uploaded to (lib/workers/exports.js uploads via lib/util/storage-backend).
      // This used to build an AWS presigned URL unconditionally, so on a GCS
      // deploy it signed against a bucket that exists only in Google Cloud
      // Storage: reported from UIndy 2026-09-02, where Download landed the
      // instructor on https://aws.amazon.com/s3/ (the marketing page that
      // https://s3.amazonaws.com/ redirects to) while the archive sat happily
      // in gs://trinket-uindy-exports.
      var storage = require('../util/storage-backend');

      return storage.signDownloadUrl(
        ...
        // Signing can fail for reasons the user cannot act on (a GCS deploy
        // whose service account lacks roles/iam.serviceAccountTokenCreator on
        // itself, for one). Say so, rather than redirecting somewhere useless.
        request.log(['error', 'export'], 'download URL signing failed: ' + err.message);
        return reply(Boom.badGateway('Could not prepare the download link'));
      });
    });
  }
};
...
function send_email_confirmation(request, new_email, key) {
  ...
  nunjucks.render('emails/confirmEmailChange', {
...
function send_email_verification(request, email, key) {
  ...
  nunjucks.render('emails/verifyEmail', {
    fullname         : request.user.fullname,
    username         : request.user.username,
    email            : email,
