    ...
    return Export.findPendingOrProcessing(userId)
      ...
      .then(function(saved) {
        ...
        if (typeof exportsQueue.hasHandlers === 'function' && !exportsQueue.hasHandlers()) {
          saved.status = 'failed';
          saved.errorMessage = 'Exports are not available on this server: no export worker is running.';
          return saved.save().then(function() {
            return request.fail({ error: saved.errorMessage, exportId: saved._id.toString() });
          });
        }
        exportsQueue.add({
