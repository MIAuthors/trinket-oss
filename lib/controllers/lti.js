            ...
            return ltiProvision.provisionUser(claims, platformShim, { isInstructor: isInstructor }).then(function (user) {
              ...
              if (body.lis_result_sourcedid && body.lis_outcome_service_url) {
                Promise.resolve(LtiOutcome.record({
                  ...
                  // Coordinates we did not hold a moment ago. If this student
                  // already submitted this assignment, that submission was
                  // unreportable when they made it — 1.1 needs a per-(student,
                  // placement) sourcedid and we had none — so the LMS grader has
                  // been showing "nothing submitted" over real work. Report it
                  // now. Gated on coordsNew so a routine relaunch is silent.
                  if (!rec || !rec.coordsNew) return null;
