...
var ltiDeepLinkCtx = require('../util/ltiDeepLinkCtx');
...
// needs-grading model.
// ---------------------------------------------------------------------------
// A deep-link context token presented by a signed-in user other than the one who
// launched it (#217, review finding 2). Rendered directly rather than as Boom 403:
// this route has no fail page, so Boom would land on the generic 50x page and lose
// the one thing the instructor needs to know — which account to sign in with.
function mismatchPage(request, reply) {
  return reply().view('lti/deep-link-mismatch.html', {
    siteName : (config.app && config.app.siteName) || 'Trinket'
  }).code(403);
}

// Sec-Fetch-Dest is sent by every current browser; an LMS dialog is an iframe.
function isFramed(request) {
  var dest = request.headers['sec-fetch-dest'];
  return dest === 'iframe' || dest === 'frame';
}

// The relay id pairs a "Continue in a new tab" page (listening in the LMS frame)
// with the tab it opened, so the tab's selection can be POSTed to the LMS from
// inside the frame (#217). Opaque, URL-safe, and validated on every read so it can
// be echoed into markup and a channel name without further thought.
var RELAY_ID_RE = /^[A-Za-z0-9_-]{8,64}$/;
function relayIdFrom(value) {
  return (typeof value === 'string' && RELAY_ID_RE.test(value)) ? value : null;
}

// Hand the LMS return to the frame instead of auto-submitting it here: see
// lib/views/lti/deep-link-relay.html. `fields` is exactly what the return form
// would have carried. JSON goes into a <script type="application/json"> block, so
// the only characters that could break out are escaped.
function relayPage(request, reply, relayId, returnUrl, fields) {
  var json = JSON.stringify({ type: 'lti-dl-return', id: relayId, url: returnUrl, fields: fields })
    .replace(/</g, '\\u003c').replace(/\u2028/g, '\\u2028').replace(/\u2029/g, '\\u2029');
  return reply().view('lti/deep-link-relay.html', {
    payloadJson : json,
    siteName    : (config.app && config.app.siteName) || 'Trinket'
  });
}
  ...
  return new Promise(function (resolve) {
    ...
    return ltiNonceStore.checkAndRecord('lti11:' + key + ':' + body.oauth_nonce, 600)
      ...
      .then(function (fresh) {
        ...
        var q = request.query || {};
        // 'page' too (#13): lti11DeepLinking puts ?page= in the content-item URL because
        // Canvas/WileyPLUS ignore `custom` on 1.1 items — dropping it here lands on the bare course.
        ['course', 'assignment', 'topic', 'page'].forEach(function (k) {
          var val = q[k];
          if (!val) return;
        ...
        var claims = {
          ...
          // 1.1 sends the parts too. Shaped under the standard OIDC names so
          // provisioning reads one set of claims for both LTI versions.
          given_name:  body.lis_person_name_given  || '',
          family_name: body.lis_person_name_family || '',
        };
          ...
          .then(function (isInstructor) {
            ...
            return ltiProvision.provisionUser(claims, platformShim, { isInstructor: isInstructor }).then(function (user) {
              ...
              if (isContentItem) {
                ...
                // Also carry the context in the URL: in an LMS iframe the cookie backing
                // this session is refused by the browser, so the session alone cannot
                // survive to the next request (#217).
                var ctx11 = ltiDeepLinkCtx.sign(request.yar.get('ltiDeepLink'), user.id);
                return reply().redirect('/lti/deep-link'
                  + (ctx11 ? '?ctx=' + encodeURIComponent(ctx11) : ''));
              }
              ...
              if (body.lis_result_sourcedid && body.lis_outcome_service_url) {
                Promise.resolve(LtiOutcome.record({
                  ...
                  // We now hold this student's coordinates for this placement.
                  // If they already submitted this assignment, that submission
                  // was unreportable when they made it — 1.1 needs a
                  // per-(student, placement) sourcedid and we had none — so the
                  // LMS grader has been showing "nothing submitted" over real
                  // work. Announce it now.
                  //
                  // Deliberately on EVERY graded launch, not only the one that
                  // first captures the token: a student who clicked their
                  // assignment before this code shipped has nothing new about
                  // their token, and gating on novelty would skip them forever.
                  // notifyOnCoordinates is idempotent via the marker it leaves
                  // on the submission.
                  //
                  // Deliberately NOT called here: the repair needs the
                  // LtiResourceLink row, and on a placement's FIRST launch that
                  // row does not exist yet -- ltiTarget.resolveTarget bootstraps
                  // it further down. Calling it here reported nothing on that
                  // first launch and made the student click twice, which is
                  // exactly the shape this fix exists to remove (an instructor
                  // deep-links work students have already submitted). It runs
                  // after resolveTarget instead; see below.
                  return null;
                }).catch(function (e) {
              ...
              var reviewId = ltiReview.targetFromRequest(request, signedParams);
              if (reviewId) {
              ...
              return ltiTarget.resolveTarget(claims, platformShim).then(function (target) {
                ...
                if (target.course) {
                    ...
                    enrollP = Promise.resolve(target.course.addUser(user, [courseRole]))
                      .then(function (res) {
                        if (res && res.alreadyListed) {
                          ...
                          if (ltiRoles.shouldUpdateCourseRole(currentRole, courseRole)) {
                  ...
                  // Land on the chosen content, exactly as the 1.3 launch does.
                  // Only the assignment case existed here (#13): a topic link
                  // resolved fine upstream and was then dropped, leaving the user
                  // on the bare course page — which for an instructor is the
                  // course's first page, often a hidden admin page. Reported on
                  // WileyPLUS (1.1 only) and not reproducible on Brightspace (1.3),
                  // because 1.3 had the branch and 1.1 did not.
                  if (target.page && target.page.lessonSlug && target.page.materialSlug) {
                    redirectPath += '#/' + target.page.lessonSlug + '/' + target.page.materialSlug;
                  } else if (target.assignment && target.assignment.lessonSlug && target.assignment.materialSlug) {
                    redirectPath += '#/' + target.assignment.lessonSlug + '/' + target.assignment.materialSlug;
                  } else if (target.topic && target.topic.lessonSlug) {
                    // The topic's first page. A bare lesson fragment is not routable
                    // by the course SPA, so an empty topic keeps the course page.
                    if (target.topic.materialSlug) {
                      redirectPath += '#/' + target.topic.lessonSlug + '/' + target.topic.materialSlug;
                    }
                  }
                }
                // The placement's LtiResourceLink exists by now -- resolveTarget
                // creates it on a first launch -- so the late-coordinate repair
                // can find it. Still best-effort and un-awaited: bookkeeping must
                // never fail or delay a student's launch.
                if (body.lis_result_sourcedid && body.lis_outcome_service_url) {
                  Promise.resolve(ltiNotifySubmission.notifyOnCoordinates(
                    iss, String(body.resource_link_id || ''), String(user.id),
                    String(body.lis_result_sourcedid)
                  )).catch(function (e) {
                    log.error('[lti11] late-coordinate repair failed (best-effort):', e && e.message);
                  });
                }
...
module.exports = {
              ...
              return ltiInstructorAuthority.resolveInstructor({ email: email0, lmsTeacher: lmsTeacher0 })
                ...
                .then(function (isInstructor0) {
                  return ltiProvision.provisionUser(claims, platform, { isInstructor: isInstructor0 }).then(function (user) {
                    ...
                    // Also carry the context in the URL: in an LMS iframe the cookie
                    // backing this session is refused by the browser, so the session
                    // alone cannot survive to the next request (#217). sign() is null
                    // only without a Tool keypair, which a 1.3 launch cannot reach —
                    // guarded anyway so the URL can never read "?ctx=null".
                    var ctx13 = ltiDeepLinkCtx.sign(request.yar.get('ltiDeepLink'), user.id);
                    return reply().redirect('/lti/deep-link'
                      + (ctx13 ? '?ctx=' + encodeURIComponent(ctx13) : ''));
                  });
                });
            }
              ...
              .then(function (isInstructor) {
                ...
                return ltiProvision.provisionUser(claims, platform, { isInstructor: isInstructor }).then(function (user) {
                  return ltiTarget.resolveTarget(claims, platform).then(function (target) {
                    ...
                    if (target.page && target.page.lessonSlug && target.page.materialSlug) {
                      // A page link (#13) addresses the material the instructor
                      // actually chose. Same two-segment routable shape as an
                      // assignment; the difference is that no line item exists,
                      // so nothing here touches AGS.
                      redirectPath = '/' + target.course.ownerSlug + '/courses/' + target.course.slug +
                        '#/' + target.page.lessonSlug + '/' + target.page.materialSlug;
                    } else if (target.assignment && target.assignment.lessonSlug && target.assignment.materialSlug) {
                      redirectPath = '/' + target.course.ownerSlug + '/courses/' + target.course.slug +
  ...
  deepLinkSelect: function(request, reply) {
    var b = request.payload || {};
    // The form carries ctx forward so a selection can still be returned to the LMS
    // when the launch's cookie was refused (#217). A VALID ctx wins over the session
    // copy: the token names WHICH deep-link request is being answered, whereas the
    // session copy is whatever the last launch or picker render in this browser
    // left behind — with two launches in flight that is the wrong one.
    //
    // The token is bound to the user who launched; this route requires a session, so
    // the check always runs here. Someone else's token is refused as such, not as
    // "expired" — the instructor is usually just signed in to a second account.
    var chk = ltiDeepLinkCtx.check(b.ctx, request.user && request.user.id);
    if (chk.mismatch) return mismatchPage(request, reply);
    var dl = chk.dl || request.yar.get('ltiDeepLink');
    if (!dl || !dl.deep_link_return_url) {
      return request.fail({ message: 'Deep linking session expired — relaunch from your LMS.' });
    }
    // One answer per request: once the selection is returned the session copy is
    // spent, so a later ctx-less POST cannot re-answer a stale launch.
    //
    // Reached from the tab the frame's "Continue in a new tab" page opened (a relay
    // id, and top-level): hand the return to the frame instead of posting it from
    // here — the LMS completes its dialog only when the return lands INSIDE the
    // frame. Framed, or without a relay id (the session path), nothing changes.
    var relayId = relayIdFrom(b.relay);
    var done = function (json) {
      request.yar.clear('ltiDeepLink');
      if (relayId && !isFramed(request)) {
        return relayPage(request, reply, relayId, json.returnUrl, json.fields || { JWT: json.jwt });
      }
      return request.success(json);
    };

    // A page (#13) is chosen from a <select>, which submits only its value — so
    // the title is composed here from the course/topic prefix plus the material's
    // own name. One read, on a path an instructor hits occasionally.
    var titleP = Promise.resolve(b.title);
    if (b.targetType === 'page' && !b.title) {
      titleP = Promise.resolve(Material.findById(b.targetId)).then(function(m) {
        var name = (m && m.name) || 'Page';
        return b.titlePrefix ? (b.titlePrefix + ' — ' + name) : name;
      }, function() { return b.titlePrefix || 'Page'; });
    }

    return titleP.then(function(resolvedTitle) {
    b.title = resolvedTitle;
    ...
    if (dl.version === '1.1') {
      var item11 = (b.targetType === 'assignment')
        ...
        : lti11DeepLinking.linkContentItem({
            courseId: b.courseId,
            lessonId: (b.targetType === 'topic' ? b.targetId : null),
            pageId:   (b.targetType === 'page'  ? b.targetId : null),
            title: b.title });
      return new Promise(function (resolve) {
    ...
    var item = (b.targetType === 'assignment')
      ...
      : ltiDeepLinking.linkContentItem({
          courseId: b.courseId,
          lessonId: (b.targetType === 'topic' ? b.targetId : null),
          pageId:   (b.targetType === 'page'  ? b.targetId : null),
          title: b.title });
    return new Promise(function(resolve) {
      ...
      return done({ returnUrl: dl.deep_link_return_url, jwt: token });
    });
    });   // titleP
  },
  ...
  deepLinkPicker: function(request, reply) {
    ...
    //  - The session is the happy path (a top-level launch, or a browser that still
    //    allows third-party cookies). The signed ?ctx= token the launch also put in
    //    the URL is the FALLBACK context: it says which deep-link request is being
    //    answered and carries no identity (lib/util/ltiDeepLinkCtx.js).
    //  - Redirecting a framed, cookieless request to /login is useless — signing in
    //    sets a cookie the browser refuses the same way, so it loops. With a valid ctx
    //    there IS a way forward: continue in a top-level tab, where the instructor's
    //    own first-party session applies. Without one (a 1.1-only deploy has no key to
    //    sign with, or the token is bad/expired) all we can do is explain the block.
    var framed = (request.headers['sec-fetch-dest'] === 'iframe'
               || request.headers['sec-fetch-dest'] === 'frame');
    var ctxToken = (request.query && request.query.ctx) || null;
    var relayId  = relayIdFrom(request.query && request.query.relay);
    // Bound to the launcher: with a session user present the binding must match;
    // without one (framed, cookieless) no check is possible and nothing proceeds.
    var chk      = ltiDeepLinkCtx.check(ctxToken, request.user && request.user.id);
    var ctxDl    = chk.dl;   // null when absent, bad, expired, or someone else's
    // A valid ctx wins: it names the request being answered. The session copy is
    // only what an earlier launch or picker render left in this browser, and with
    // two launches in flight (or a top-level tab reused across launches) it is stale.
    var dl = ctxDl || request.yar.get('ltiDeepLink');

    if (!request.user) {
      if (framed && ctxDl) {
        // A fresh relay id: the frame listens on it, the tab reports back on it.
        var newRelay = crypto.randomBytes(12).toString('base64url');
        return reply().view('lti/deep-link-continue.html', {
          continueUrl : '/lti/deep-link?ctx=' + encodeURIComponent(ctxToken) + '&relay=' + newRelay,
          relayId     : newRelay,
          siteName    : (config.app && config.app.siteName) || 'Trinket'
        });
      }
      ...
      if (ctxDl) {
        // Top-level and signed out (the "Continue in a new tab" link, or a bookmarked
        // picker URL): an ordinary sign-in works here, so send them through it and
        // back to this same picker URL. Same `next` hand-off the login form honours.
        request.yar.set('next', '/lti/deep-link?ctx=' + encodeURIComponent(ctxToken)
          + (relayId ? '&relay=' + relayId : ''));
        return reply().redirect('/login');
      }
      // Top-level, signed out, nothing to continue on: someone opened the bare URL.
      return request.fail({ message: 'Deep linking session expired — relaunch from your LMS.' });
    }
    if (chk.mismatch) return mismatchPage(request, reply);
    ...
    // Rendering from ctx: bring the session into agreement, so a later request in
    // this same tab that arrives without the token (a form that lost it, a reload
    // of the response page) answers the same launch, not an older one.
    if (ctxDl) request.yar.set('ltiDeepLink', ctxDl);
    ...
    return new Promise(function(resolve, reject) {
      ...
      return Promise.all(courses.map(function(course) {
        if (bothMode) {
          ...
          return Promise.all((course.lessons || []).map(function(lessonId) {
            ...
            // Draft topics are invisible to students, so linking one from the LMS
            // would send a class to content they cannot see (#13).
            var topics = lessons
              .filter(function(lesson) { return lesson && lesson.slug && !lesson.isDraft; })
              .map(function(lesson) { return { lessonId: lesson.id, title: lesson.name, slug: lesson.slug }; });
            // ONE material pass per lesson, deriving BOTH datasets. Pages were
            // previously discarded here (#13) — an instructor could only link a
            // whole topic, which lands on the topic's first page rather than the
            // one they meant. Reading the materials twice would double the
            // Firestore reads for no reason (CLAUDE.md cost rule).
            return Promise.all(lessons.filter(function(l) { return !l.isDraft; }).map(function(lesson) {
              return Promise.all((lesson.materials || []).map(function(materialId) {
                return Promise.resolve(Material.findById(materialId)).then(function(m) {
                  if (!m) return null;
                  if (m.isDraft) return null;   // draft page: students cannot see it
                  return { lessonId: lesson.id, materialId: m.id, title: m.name,
                           type: m.type, slug: m.slug };
                }, function() { return null; });
              }));
            })).then(function(perLesson) {
              var all = [].concat.apply([], perLesson).filter(Boolean);
              var assignments = all
                .filter(function(m) { return m.type === 'assignment'; })
                .map(function(m) { return { materialId: m.materialId, title: m.title }; });
              // A page needs a slug: the launch lands on #/{lessonSlug}/{materialSlug},
              // and a material without one cannot be addressed.
              var pagesByLesson = {};
              all.filter(function(m) { return m.type !== 'assignment' && m.slug; })
                 .forEach(function(m) {
                   (pagesByLesson[m.lessonId] = pagesByLesson[m.lessonId] || [])
                     .push({ materialId: m.materialId, title: m.title });
                 });
              topics.forEach(function(t) { t.pages = pagesByLesson[t.lessonId] || []; });
        ...
        if (contentMode) {
          ...
          // Each topic carries its pages (#13). This costs one material read per
          // material, which contentMode did not previously pay — the price of
          // being able to link a page at all. Assignments are excluded here:
          // link_selection mode is for non-graded placement.
          return Promise.all((course.lessons || []).map(function(lessonId) {
            return Promise.resolve(Lesson.findById(lessonId)).then(function(lesson) {
              if (!lesson || !lesson.slug || lesson.isDraft) return null;
              return Promise.all((lesson.materials || []).map(function(materialId) {
                return Promise.resolve(Material.findById(materialId)).then(function(m) {
                  return (m && m.slug && !m.isDraft && m.type !== 'assignment')
                    ? { materialId: m.id, title: m.name } : null;
                }, function() { return null; });
              })).then(function(pages) {
                return { lessonId: lesson.id, title: lesson.name, slug: lesson.slug,
        ...
        return Promise.all((course.lessons || []).map(function(lessonId) {
          return Promise.resolve(Lesson.findById(lessonId)).then(function(lesson) {
            if (!lesson || lesson.isDraft) return [];   // draft topic: invisible to students
            return Promise.all((lesson.materials || []).map(function(materialId) {
              return Promise.resolve(Material.findById(materialId)).then(function(m) {
                return (m && m.type === 'assignment' && !m.isDraft)
                  ? { materialId: m.id, title: m.name } : null;
              });
            }));
          });
      ...
      return request.success({
        courses: view,
        // Threaded into the select forms so the flow survives a refused cookie (#217).
        // Only a token that verified — a stale one would just be dead weight.
        ctx: ctxDl ? ctxToken : null,
        // Pairs this tab with the frame that opened it, so select can hand the LMS
        // return back to the frame (#217).
        relay: relayId,
        mode: mode,
