  ...
  viewClass : function(request, reply) {
    ...
    ||  (request.user && request.user.hasPermission('view-course-content', 'course', { id : course.id })))) {
      ...
      if (request.user) {
        ...
        // "copy this course" is only meaningful for users who can actually
        // create new courses. Even if the course is marked open/copyable,
        // a student or other non-creator shouldn't see the button.
        result.canCopy = !result.canEdit
          && (course.globalSettings.copyable || course.globalSettings.courseType === "open")
          && helpers.userCanCreateCourse(request.user);
      }
      ...
      return Course.findFeaturedForUser(request.user)
        .then(function(courses) {
          courses = _.map(courses, function(course) {
            ...
            // This fallback shelf renders name/description/url only, so the
            // lesson id list is dead weight on every render. (It was also the
            // payload that used to blow up here: this route content-negotiates,
            // so a client sending `Accept: application/json` got the JSON branch
            // and a 500 from the ObjectId-recursion bug now fixed centrally in
            // ObjectUtils.serialize — a browser asking for HTML never saw it.)
            ...
            .then(function(course) {
              if (request.user) {
                if (request.user.email && invitation.email
                    && request.user.email.toLowerCase() !== invitation.email.toLowerCase()) {
                  request.yar.flash("warning",
                    "This invitation is for <strong>" + invitation.email + "</strong>, but you're signed in as <strong>" + request.user.email + "</strong>. "
                    + "Please sign out and sign back in with the email address your instructor used to add you to the course.");
