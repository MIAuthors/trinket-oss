var config         = require('config'),
    ...
    courseEnrollment = require('../util/courseEnrollment');
...
module.exports = {
  ...
  loginPage: function(request, reply) {
    ...
    return request.success({
      ...
      authProviders: (config.auth.firebase && Array.isArray(config.auth.firebase.providers))
        ? config.auth.firebase.providers
        : ['google']
    });
  },
  ...
  session: async function(request, reply) {
    ...
    if (!user) {
      // The Firebase UID isn't linked to any account yet, so anything below
      // (email fallback match, account creation, or linking) trusts the token's
      // EMAIL claim. Firebase issues email/password tokens with
      // email_verified:false immediately on signup, so an unverified email here
      // could match + link onto an existing account — account takeover
      // (GHSA-w66h-rw9x-7h24). Require a verified email before any email-based
      // resolution. Google tokens are always verified; already-linked users hit
      // the UID lookup above and never reach here. See docs/design/firebase-email-verification.md.
      if (decoded.email_verified !== true) {
        // Don't claim a link "has been sent" — the SERVER never sends one;
        // the login page emails it on fresh signup only. The page shows its
        // own accurate notice for code EMAIL_NOT_VERIFIED; this message is
        // for logs and non-browser clients.
        var verErr = Boom.forbidden(
          'Your email address (' + email + ') is not verified yet. ' +
          'Check your inbox for the verification link, then sign in again.'
        );
        verErr.output.payload.code = 'EMAIL_NOT_VERIFIED';
        return reply(verErr);
      }
    ...
    // Seed the site 'admin' role from the admin-email allowlist (idempotent; writes only when the
    // role is newly added). Gates check hasRole('admin'); the email list is just the seed source.
    await siteAdmin.ensureSeedAdminRole(user);

    // Refresh the instructor flag from the authorized allowlist (idempotent; writes only when
    // flipping false -> true). isInstructor is otherwise set only at first Firebase signup, so
    // this heals accounts made via other paths or authorized after signup.
    await instructorAuth.ensureInstructorFlag(user);

    // Accept pending course invitations + enroll — for EVERY login (new AND existing
    // users), so an existing Trinket user added to a course via "Add Students" gets
    // enrolled on their next login, not only on first signup (MIAuthors #10).
    // Idempotent (only pending/sent invitations; addUser short-circuits if enrolled).
    enrolledCourseFlash = await courseEnrollment.acceptPendingInvitations(user);
    ...
    // New signups land on /welcome (which shows the "account created" notice);
    // returning users go to /home. Previously everyone hit /welcome, so existing
    // users saw "Welcome! Your account has been created." on every login.
    //
    // .code(200) is REQUIRED, not decorative: a bare `return reply(obj)` returns
    // the shim's chainable builder to hapi, which serializes it to `{}` (all its
    // props are functions) — the browser then never sees `redirect` and
    // login-firebase.html falls back to a hardcoded '/welcome'. A terminal chain
    // call (.code/.header/…) is what resolves the real {status, redirect} body.
    return reply({ status: 'success', redirect: next || (isNewUser ? '/welcome' : '/home') }).code(200);
  },
  ...
  googleCallback : function(request, h) {
    ...
    .then(function(user) {
      // Seed the site 'admin' role from the allowlist before login (same as the Firebase path).
      return Promise.resolve(siteAdmin.ensureSeedAdminRole(user)).then(function() { return user; });
    })
    .then(function(user) {
      // Refresh the instructor flag (same as the Firebase path). The Google-OAuth signup path
      // never set isInstructor, so without this every google-sourced instructor stays false.
      return Promise.resolve(instructorAuth.ensureInstructorFlag(user)).then(function() { return user; });
    })
    .then(function(user) {
