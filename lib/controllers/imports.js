...
// Heuristic for a Python-2 print STATEMENT (`print x`, `print "x"`, `print >>f`)
// at the start of a logical line. Python-3 print() has no space before '(', so
// it is excluded. Only used to flag imported legacy-"python" trinkets whose code
// won't run as-is on Python 3, so the import summary can tell the user which
// ones need a small update (e.g. `print x` -> `print(x)`). A stray match inside
// a triple-quoted string is harmless — worst case an over-cautious heads-up.
var PY2_PRINT_RE = /(^|\n)[ \t]*print[ \t]+[^(=\s]/;
function looksLikePython2(code) {
  return typeof code === 'string' && PY2_PRINT_RE.test(code);
}
