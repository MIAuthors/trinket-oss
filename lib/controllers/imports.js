...
function importCourse(request, reply) {
  ...
  return readUploadedFile(request.payload.file)
    ...
    .catch(function(err) {
      // request.fail() wraps any Error/Boom as a 500 badImplementation, masking the
      // real status; reply(err) passes a Boom through with its own code (matches
      // importTrinkets above). Plain-object failures still go via request.fail.
      if (err.isBoom) return reply(err);
