...
// #108: the per-trinket runtime override. The page rebuilds its iframe src from a
// fixed allowlist of query parameters, so `runtime` has to be carried explicitly
// or it is dropped between the shared page URL and the embed that reads it.
// Only the two values the runtime router understands are echoed back into that
// src — anything else is ignored rather than reflected.
