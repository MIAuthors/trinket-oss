...
// #108/#128: the per-trinket runtime override. Shared by two callers — the
// `?runtime=` query param (index, below) and the persisted `settings.runtime`
// (sanitizeSettings, below) — so it whitelists for both. Don't "specialize"
// this for one caller; add a second function instead if their rules ever
// diverge. Only the two values the runtime router understands pass through;
// anything else becomes '' ("follow the deploy" / not reflected into the src).
...
module.exports = {
  ...
  update : function(request, reply) {
    ...
    // #128: this is the ONLY write path a stored trinket.settings.runtime can
    // reach the shared Trinket document through (draft/autosave write to the
    // Draft doc or a request-scoped trinket, not this one). Without whitelisting,
    // Firestore stores an arbitrary string verbatim (unmatched <option>, no way
    // for the author to clear it) and Mongo's enum validator rejects the whole
    // .save() — silently discarding a code save, since the client's save()
    // swallows the error. See sanitizeSettings above.
