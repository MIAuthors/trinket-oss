var mailer        = require('../util/mailer'),
    ...
    JSZip         = require("jszip"),
    sanitizeSettingsUtil = require('../util/sanitizeSettings'),
    validRuntime  = sanitizeSettingsUtil.validRuntime,
    sanitizeSettings = sanitizeSettingsUtil.sanitizeSettings;
...
// #108/#128: validRuntime and sanitizeSettings moved to
// ../util/sanitizeSettings (required above) so lib/controllers/imports.js
// can share the same whitelist instead of carrying its own copy. Behaviour
// is unchanged — see that file for the up-to-date comments.

// #139: the canonical redirect must carry the query string. `/library/trinkets/<id>`
// is the URL an author has in the address bar while looking at their own trinket,
// so it is the one they copy and add options to — and dropping the query silently
// discarded outputOnly, toggleCode, runMode, runOption and runtime alike. The
// receiving page whitelists what it acts on, so forwarding the raw query adds no
// new surface.
function canonicalTrinketPath(request, trinket) {
  var path  = '/' + trinket.lang + '/' + trinket.shortCode;
  var query = request.url && request.url.search ? request.url.search : '';
  return path + query;
}
...
module.exports = {
  index : function(request, reply) {
    ...
    var runtime           = validRuntime(request.query.runtime);
      ...
      if (trinketId) {
        Trinket.findById(trinketId, function(err, trinket) {
          if (trinket) {
            ...
            return reply().redirect(canonicalTrinketPath(request, trinket));
          }
  ...
  list : function(request, reply) {
    ...
    return getUserId()
      ...
      .then(function(trinkets) {
        // Normalize to plain objects FIRST: on the mongoose backend these are
        // Documents, where (a) assigning non-schema fields (totalViews, id,
        // lowerName...) is silently DROPPED at serialization, and (b) the
        // unset nested path `folder` reads back as a truthy {} — which would
        // make the folderless filter below drop EVERY unfiled trinket.
        // toObject() (minimize) turns that {} into undefined. Firestore
        // docs' toObject() includes id/_id already.
        trinkets = trinkets.map(function(t) {
          return (typeof t.toObject === 'function') ? t.toObject() : t;
        });

        // Scope + name + date filters in JS (Firestore doesn't handle nested
        // $exists, so this list has always filtered in-process).
        ...
        // Date filter over lastUpdated. An explicit after/before range takes
        // precedence over the preset. 'before' is inclusive of the whole named
        // day (a date input gives midnight, so we compare against next midnight).
        var luOf = function(t) {
          return t.lastUpdated ? (t.lastUpdated.toDate ? t.lastUpdated.toDate() : new Date(t.lastUpdated)) : null;
        };
        var after  = request.query.updatedAfter  ? new Date(request.query.updatedAfter)  : null;
        var before = request.query.updatedBefore ? new Date(request.query.updatedBefore) : null;
        var beforeEnd = before ? new Date(before.getTime() + 24 * 60 * 60 * 1000) : null;

        if (after || beforeEnd) {
          trinkets = trinkets.filter(function(t) {
            var lu = luOf(t);
            if (!lu) return false;
            if (after && lu < after) return false;
            if (beforeEnd && lu >= beforeEnd) return false;
            return true;
          });
        } else if (within !== 'all') {
  ...
  update : function(request, reply) {
    ...
    // #128: settings.runtime can reach a stored Trinket document through several
    // write paths — this update(), the draft/autosave paths below, and the
    // imports.js replace/create paths (a user-uploaded zip's metadata.json) —
    // and every one of them must sanitize. Without whitelisting, Firestore
    // stores an arbitrary string verbatim (unmatched <option>, no way for the
    // author to clear it) and Mongo's enum validator rejects the whole .save()
    // — silently discarding a code save, since the client's save() swallows the
    // error. See ../util/sanitizeSettings.
    if (request.payload.settings) {
      request.payload.settings = sanitizeSettings(request.payload.settings);
    }
  ...
  create : function(request, reply) {
    ...
    if (request.user) {
      ...
      // A logged-in user owns the trinkets they create. Previously _owner was
      // set only when the client happened to send ?library, so any create that
      // missed that param produced an ownerless trinket — absent from the user's
      // library (list() matches on _owner) and isOwner=false in the embed
      // (Instructions tab hidden, "Remix" shown instead of "Save"). Own it
      // unconditionally here (see issue #35).
  ...
  updateMetrics : function(request, reply) {
    ...
    if (!metric) {
      ...
      // Promise style (like the branch below): a callback-driven Model.findById
      // returned through the pre-handler shim double-executes on the Mongo backend.
      return Trinket.findById(request.params.trinketId)
        .then(function(trinket) { return request.success({data: trinket}); })
        .catch(reply);
    }
  ...
  snapshot : function(request, reply) {
    ...
    return reply({ status : "success" }).code(200);
  },
...
function resolveLegacy(request, reply, isEmbed) {
  ...
  return Trinket.findByLegacyShortCodes([code])
    .then(function(trinkets) {
      ...
      var queryPart  = new URLSearchParams(request.query).toString();
