...
module.exports = {
  ...
  create : function(request, reply) {
    ...
    if (request.user) {
      ...
      // library, and isOwner=false in the embed (Instructions tab hidden, "Remix"
      // shown instead of "Save"). Own it unconditionally here (see issue #35).
  ...
  snapshot : function(request, reply) {
    // DIAGNOSTIC LOGGING (temporary): surface why screenshots aren't storing.
    var _snapBucket = (config.gcs && config.gcs.buckets && config.gcs.buckets.snapshots && config.gcs.buckets.snapshots.name) || '(unset)';
    if (request.payload.snapshotData) {
      var img  = request.payload.snapshotData;
      var data = img.replace(/^data:image\/\w+;base64,/, "");
      var buf  = Buffer.from(data, 'base64');

      // Guard against blank/raced captures: a WebGL toDataURL() that read an
      // already-cleared drawing buffer yields "data:," (decodes to a few bytes)
      // or a tiny transparent PNG. Storing it would overwrite a good thumbnail
      // with a broken one. Real screenshots are several KB, so require a valid
      // PNG data URL of plausible size. (Root cause is the glowscript raw-
      // toDataURL fallback in embed/glowscript.html — the client fix lives there.)
      var MIN_SNAPSHOT_BYTES = 1024;
      if (!/^data:image\/png;base64,/.test(img) || buf.length < MIN_SNAPSHOT_BYTES) {
        log.info('[snapshot] SKIP blank/invalid capture (' + buf.length + ' bytes) — keeping existing snapshot for ' + request.pre.trinket.shortCode);
      ...
      log.info('[snapshot] uploading ' + imagename + ' (' + buf.length + ' bytes) -> gcs bucket ' + _snapBucket);
      return StorageUtil.uploadSnapshot(imagename, buf)
        .then(function(snapshotUrl) {
          log.info('[snapshot] upload OK -> ' + snapshotUrl);
          request.pre.trinket.snapshot = snapshotUrl;
          return request.pre.trinket.save();
        })
        .then(request.success)
        .catch(function(err) {
          // Snapshot upload is non-critical; log and succeed anyway. Log the real
          // message + stack (winston's simple format drops a bare Error to blank).
          log.error('[snapshot] upload FAILED to bucket ' + _snapBucket + ': ' +
            (err && (err.stack || err.message)) + ' | code=' + (err && (err.code || err.statusCode)));
          return request.success({ status: 'success' });
        });
    }
    log.info('[snapshot] no snapshotData in payload (bucket would be ' + _snapBucket + ') — nothing uploaded');
