...
`submitCourseDirect` on a real HTTP **501** (the `isSignedUploadAvailable() === false`
opt-out case in `getImportUploadUrl`, `lib/controllers/imports.js`),
not on a signing error. That's intentional — a file large
...
Cloud Run memory allocation rather than assuming it:

```bash
gcloud run services describe trinket --region us-central1 --project <PROJECT> \
