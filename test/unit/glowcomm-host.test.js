...
//
// glowcomm_host.js is a FETCHED asset (pinned upstream release — see the
// Dockerfile's VPYTHON_WHEEL_RELEASE block), not a tracked file. On a fresh
// checkout run `scripts/sync-vpython-worker.sh` once; until then this suite
// skips loudly rather than failing on a missing file.
const fs = require('node:fs');
const GLOWCOMM_HOST = require.resolve ? __dirname + '/../../public/components/vpython-worker/glowcomm_host.js' : '';
const HAVE_ASSET = fs.existsSync(GLOWCOMM_HOST);
const { createGlowFrontend } = HAVE_ASSET ? require(GLOWCOMM_HOST) : { createGlowFrontend: null };
...
const describeIfAsset = HAVE_ASSET ? describe : describe.skip;
describeIfAsset('createGlowFrontend [run scripts/sync-vpython-worker.sh if skipped]', () => {
...
describeIfAsset('createGlowFrontend event channel', () => {
